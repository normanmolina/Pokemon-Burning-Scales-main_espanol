...
class PokemonBagScreen
  ...
  def pbStartScreen
    ...
    loop do
      ...
      itemname = itm.name
      command = @scene.pbShowCommands(_INTL("{1} está seleccionado.",itemname),commands)
      if cmdRead>=0 && command==cmdRead   # Read mail
        ...
        if $Trainer.pokemon_count == 0
          ...
          @scene.pbDisplay(_INTL("El {1} no se puede retener.",itemname))
        else
          pbFadeOutIn {
        ...
        if qty>1
          helptext = _INTL("¿Cuántos {1} tirar?",itm.name_plural)
          qty = @scene.pbChooseNumber(helptext,qty)
        end
        if qty>0
          itemname = itm.name_plural if qty>1
          if pbConfirm(_INTL("¿Está bien tirar {1} {2}?",qty,itemname))
            pbDisplay(_INTL("Tiró {1} {2}.",qty,itemname))
        ...
        loop do
          command = @scene.pbShowCommands(_INTL("¿Qué hacer con {1}?",itemname),[
            ...
            _INTL("Hacer un regalo misterioso"),
            _INTL("Cancelar")
            ],command)
          case command
          ### Cancel ###
          ...
          when 0
            ...
            newqty = pbMessageChooseNumber(
               _INTL("Elija una nueva cantidad de {1} (máx.#{Settings::BAG_MAX_PER_SLOT}).",itemplural),params) { @scene.pbUpdate }
            if newqty>qty
  ...
  def pbWithdrawItemScreen
    ...
    loop do
      ...
      if qty>1 && !itm.is_important?
        qty = @scene.pbChooseNumber(_INTL("¿Cuantos quieres retirar?"),qty)
      end
      next if qty<=0
      if @bag.pbCanStore?(item,qty)
        if !storage.pbDeleteItem(item,qty)
          raise "No se pueden eliminar elementos del almacenamiento"
        end
        if !@bag.pbStoreItem(item,qty)
          raise "No se pueden retirar artículos del almacenamiento"
        end
        @scene.pbRefresh
        dispqty = (itm.is_important?) ? 1 : qty
        itemname = (dispqty>1) ? itm.name_plural : itm.name
        pbDisplay(_INTL("Retiró {1} {2}.",dispqty,itemname))
      else
      ...
      if qty>0
        if !storage.pbCanStore?(item,qty)
          ...
          if !@bag.pbDeleteItem(item,qty)
            raise "No puedo eliminar elementos de la mochila"
          end
          if !storage.pbStoreItem(item,qty)
            raise "No se pueden depositar objetos en el almacenamiento"
          end
          @scene.pbRefresh
          dispqty  = (itm.is_important?) ? 1 : qty
          itemname = (dispqty>1) ? itm.name_plural : itm.name
          pbDisplay(_INTL("Depositado {1} {2}.",dispqty,itemname))
        end
      end
    end
    @scene.pbEndScene
  ...
  def pbTossItemScreen
    ...
    loop do
      ...
      itm = GameData::Item.get(item)
      if itm.is_important?
        @scene.pbDisplay(_INTL("¡Eso es demasiado importante para tirarlo!"))
        next
      end
      qty = storage.pbQuantity(item)
      itemname       = itm.name
      ...
      if qty>1
        qty=@scene.pbChooseNumber(_INTL("¿Cuántas {1} tirar?",itemnameplural),qty)
      end
      if qty>0
