...
class DiegoWTsStarterSelection
  ...
  def pbChooseBall
    ...
    if @pokemon.type2 != @pokemon.type1 && StarterSelSettings::TYPE2COLOR
      @sprites["textbox"].text = _INTL("<ac>¿Elegirás a #{@pkmnname}, <br>el Pokémon de tipo dual #{type1}/#{type2}?</ac>")
    elsif StarterSelSettings::TYPE2COLOR
      @sprites["textbox"].text = _INTL("<ac>¿Elegirás a #{@pkmnname}, el Pokémon de tipo #{type1}?</ac>")
    else
      @sprites["textbox"].text = _INTL("<ac>¿Elegirás a #{@pkmnname}, el Pokémon de tipo #{type1}?</ac>")
    end
