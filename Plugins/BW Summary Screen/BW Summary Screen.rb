...
#=============================================================================
  ...
  def drawPage(page)
    ...
    if SUMMARY_B2W2_STYLE
      textpos = [
        ...
        [_INTL("Objeto"),298,314,0,base,shadow]
      ]
    else
      textpos = [
        ...
        [@pokemon.name,354,40,0,Color.new(90,82,82),Color.new(165,165,173)],
        [@pokemon.level.to_s,346,72,0,Color.new(90,82,82),Color.new(165,165,173)],
        [_INTL("Objeto"),298,316,0,base,shadow]
      ]
    end
    ...
    if @pokemon.hasItem?
      ...
      textpos.push([_INTL("Ninguno"),290,344,0,base,shadow])
    end
  ...
  def drawPageOne
    ...
    if @pokemon.owner.name.empty?
      if SUMMARY_B2W2_STYLE
        textpos.push([_INTL("ALQUILER"),164,156,0,Color.new(90,82,82),Color.new(165,165,173)])
        textpos.push(["?????",164,186,0,Color.new(90,82,82),Color.new(165,165,173)])
      else
        textpos.push([_INTL("ALQUILER"),164,158,0,Color.new(90,82,82),Color.new(165,165,173)])
    ...
    if @pokemon.shadowPokemon?
      textpos.push([_INTL("Medidor de corazón"),33,222,0,base,shadow])
      heartmessage = [_INTL("¡La puerta de su corazón está abierta! ¡Deshacer el bloqueo final!"),
                      ...
                      _INTL("La puerta de su corazón está bien cerrada.")][@pokemon.heartStage]
      ...
      if SUMMARY_B2W2_STYLE
        textpos.push([_INTL("Puntos de Exp."),34,220,0,base,shadow])
        ...
        textpos.push([_INTL("Para sig. Lv."),34,284,0,base,shadow])
        ...
        textpos.push([_INTL("Puntos de Exp."),34,222,0,base,shadow])
        # Changed the Positon of No. of Exp
        textpos.push([@pokemon.exp.to_s_formatted,215,254,2,Color.new(90,82,82),Color.new(165,165,173)])
        textpos.push([_INTL("Para sig. Lv."),34,286,0,base,shadow])
        # Changed the Positon of No. of Exp to Next Level
  ...
  def drawPageOneEgg
    ...
    if mapname && mapname != ""
      # Changed the color of the text, to the one used in BW
      memo += _INTL("<c3=404040,B0B0B0>Un misterioso Huevo Pokémon recibido de<c3=0000D6,7394FF>{1}<c3=404040,B0B0B0>.\n",mapname)
    else
      # Changed the color of the text, to the one used in BW
      memo += _INTL("<c3=404040,B0B0B0>Un misterioso Huevo Pokémon.\n",mapname)
    end
    memo += "\n" # Empty line
    # Write Egg Watch blurb
    memo += _INTL("<c3=404040,B0B0B0>\"El reloj del huevo\"\n")
    eggstate = _INTL("Parece que este huevo tardará mucho en salir del cascarón.")
    eggstate = _INTL("¿Qué saldrá de esto? No parece estar cerca de eclosionar.") if @pokemon.steps_to_hatch < 10200
    eggstate = _INTL("Parece moverse de vez en cuando. Puede estar cerca de eclosionar.") if @pokemon.steps_to_hatch < 2550
    eggstate = _INTL("¡Se pueden escuchar sonidos provenientes del interior! ¡Eclosionará pronto!") if @pokemon.steps_to_hatch < 1275
    memo += sprintf("<c3=404040,B0B0B0>%s\n",eggstate)
    # Draw all text
  ...
  def drawPageTwo
    ...
    if showNature
      ...
      # Changed the color of the text, to the one used in BW
      memo += _INTL("Naturaleza <c3=0000d6,7394ff>{1}<c3=404040,B0B0B0>.\n",natureName)
    end
    # Write date received
    if @pokemon.timeReceived
      date  = @pokemon.timeReceived.day
    ...
    mettext = [_INTL("Conocido al Lv. {1}.",@pokemon.obtain_level),
               _INTL("Huevo recibido."),
               _INTL("Intercambiado al Lv. {1}.",@pokemon.obtain_level),
               "",
               _INTL("Tuvo un fatídico encuentro al Lv. {1}.",@pokemon.obtain_level)
    ...
    if showNature
      ...
      characteristics = {
        :HP              => [_INTL("Le encanta comer"),
                             _INTL("Toma muchas siestas"),
                             _INTL("Asiente mucho"),
                             _INTL("Dispersa cosas a menudo"),
                             _INTL("Le gusta relajarse.")],
        :ATTACK          => [_INTL("Orgulloso de su poder").
                             _INTL("Le gusta dar vueltas"),
                             _INTL("Un poco de mal genio"),
                             _INTL("Le gusta pelear"),
                             _INTL("Templado.")],
        :DEFENSE         => [_INTL("Cuerpo resistente"),
                             _INTL("Capaz de recibir golpes"),
                             _INTL("Altamente persistente"),
                             _INTL("Buena resistencia"),
                             _INTL("Buena perseverancia.")],
        :SPECIAL_ATTACK  => [_INTL("Muy curioso"),
                             _INTL("Travieso"),
                             _INTL("Completamente astuto"),
                             _INTL("A menudo perdido en sus pensamientos"),
                             _INTL("Muy quisquilloso.")],
        :SPECIAL_DEFENSE => [_INTL("Voluntad fuerte"),
                             _INTL("Algo vanidoso"),
                             _INTL("Fuertemente desafiante"),
                             _INTL("Odia perder"),
                             _INTL("Algo terco.")],
        :SPEED           => [_INTL("Le gusta correr"),
                             _INTL("Alerta de sonidos"),
                             _INTL("Impetuoso y tonto."),
                             _INTL("Algo de payaso"),
                             _INTL("Rápido para huir.")]
      }
...
# Write various bits of text
    if SHOW_EV_IV
      textpos = [
        [_INTL("PS"),64,70,0,Color.new(255,255,255),statshadows[:HP]],
        ...
        [_INTL("Ataque"),16,120,0,Color.new(255,255,255),statshadows[:ATTACK]],
        ...
        [_INTL("Defensa"),16,152,0,Color.new(255,255,255),statshadows[:DEFENSE]],
        ...
        [_INTL("Ataque Sp."),16,184,0,Color.new(255,255,255),statshadows[:SPECIAL_ATTACK]],
        ...
        [_INTL("Defensa Sp."),16,216,0,Color.new(255,255,255),statshadows[:SPECIAL_DEFENSE]],
        ...
        [_INTL("Velocidad"),16,248,0,Color.new(255,255,255),statshadows[:SPEED]],
        ...
        [_INTL("Habilidad"),38,282,0,Color.new(255,255,255),Color.new(165,165,173)],
      ]
    else
      textpos = [
        [_INTL("PS"),64,70,0,Color.new(255,255,255),statshadows[:HP]],
  ...
  def drawPageFourSelecting(move_to_learn)
    ...
    if move_to_learn || SUMMARY_B2W2_STYLE
      ...
      textpos = [
        [_INTL("MOVIMIENTOS"),26,2,0,base,shadow],
        [_INTL("CATEGORIA"),20,116,0,base,shadow],
        [_INTL("PODER"),20,148,0,base,shadow],
        [_INTL("PRECISION"),20,180,0,base,shadow]
      ]
  ...
  def drawPageFive
    ...
    # Write various bits of text
    textpos = [
       [_INTL("No. de Listones:"),38,296,0,Color.new(255,255,255),Color.new(165,165,173)],
    ...
    loop do
      ...
      if redraw
        ...
        textpos = [
           ...
           [_INTL("Cancelar"),366,290,2,base,shadow]
        ]
