...
def pbSurf
  ...
  if pbConfirmMessage(_INTL("El agua es de un azul profundo...\n¿Te gustaría surfear en ella?"))
    ...
    pbMessage(_INTL("{1} usó {2}!", speciesname, movename))
    pbCancelVehicles
...
def pbDive
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_DIVE,false) || (!$DEBUG && !movefinder)
    pbMessage(_INTL("El mar es profundo aquí. Un Pokémon puede sumergirse bajo el agua."))
    return false
  end
  if pbConfirmMessage(_INTL("El mar es profundo aquí. ¿Te gustaría usar Dive?"))
    if movefinder
      speciesname = movefinder.name
...
def pbSurfacing
  ...
  movefinder = $Trainer.get_pokemon_with_move(move)
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_DIVE,false) || (!$DEBUG && !movefinder)
    pbMessage(_INTL("La luz se filtra desde arriba. Un Pokémon puede salir a la superficie aquí."))
    return false
  end
  if pbConfirmMessage(_INTL("La luz se filtra desde arriba. ¿Te gustaría usar Dive?"))
...
HiddenMoveHandlers::UseMove.add(:STRENGTH,proc {|move,pokemon|
  if !pbHiddenMoveAnimation(pokemon,false)
    pbMessage(_INTL("{1} usó {2}!\1",pokemon.name,GameData::Move.get(move).name))
  end
  pbMessage(_INTL("¡La fuerza de {1} hizo posible mover rocas!",pokemon.name))
...
def pbHeadbutt(event = nil)
  ...
  movefinder = $Trainer.get_pokemon_with_move(move)
  if !$DEBUG && !movefinder
    pbMessage(_INTL("Un Pokémon podría estar en este árbol. Tal vez un Pokémon podría sacudirlo."))
    return false
  end
