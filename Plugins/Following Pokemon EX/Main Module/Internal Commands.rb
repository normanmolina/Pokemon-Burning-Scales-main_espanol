module FollowingPkmn
  ...
  def self.item(item, quantity = 1, message = nil)
    ...
    message = _INTL("{1} parece estar sosteniendo algo...") if nil_or_empty?(message)
    ...
    if $PokemonBag.pbStoreItem(item, quantity)   # If item can be picked up
      ...
      if item == :LEFTOVERS
        pbMessage(_INTL("\\me[{1}]{3} encontró algunos \\c[1]{2}\\c[0]!\\wtnp[30]", meName, itemname, pokename))
        ...
        pbMessage(_INTL("\\me[{1}]{3} encontró un \\c[1]{2}\\c[0]!\\wtnp[30]", meName, itemname, pokename))
      else
        pbMessage(_INTL("\\me[{1}]{3} encontró un \\c[1]{2}\\c[0]!\\wtnp[30]", meName, itemname, pokename))
      end
      pbMessage(_INTL("Guardas el {1}\\nen el <icon=bagPocket{2}>\\c[1]{3} Pocket\\c[0].",
    ...
    # Can't add the item
    if item == :LEFTOVERS
      pbMessage(_INTL("{1} encontró algunos \\c[1]{2}\\c[0]!\\wtnp[30]", pokename, itemname))
    elsif item.is_machine?   # TM or HM
      ...
      pbMessage(_INTL("{1} encontró un \\c[1]{2}\\c[0]!\\wtnp[30]", pokename, itemname))
    else
      pbMessage(_INTL("{1} encontró un \\c[1]{2}\\c[0]!\\wtnp[30]", pokename, itemname))
    end
