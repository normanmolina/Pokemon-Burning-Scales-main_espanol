...
class MenuEntryExitSafari < MenuEntry
  ...
  def selected(menu)
    menu.pbHideMenu
    if pbConfirmMessage(_INTL("¿Te gustaría dejar el juego Safari ahora mismo?"))
      $game_temp.in_menu = false
...
class MenuEntryExitBugContest < MenuEntry
  ...
  def selected(menu)
    menu.pbHideMenu
    if pbConfirmMessage(_INTL("¿Te gustaría finalizar el Concurso ahora?"))
      $game_temp.in_menu = false
...
class MenuEntryQuit < MenuEntry
  ...
  def selected(menu)
    menu.pbHideMenu
    if pbConfirmMessage(_INTL("¿Está seguro que quiere salir del juego?"))
      scene = PokemonSave_Scene.new
      screen = PokemonSaveScreen.new(scene)
      screen.pbSaveScreen
      menu.pbEndScene
