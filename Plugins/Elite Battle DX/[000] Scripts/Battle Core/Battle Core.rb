...
class PokeBattle_Battle
  ...
  def pbStartBattleSendOut(sendOuts)
    ...
    # Wild battlers
    memb = []; text = wildBattle? ? "¡Vaya! Un salvaje" : "Eres desafiado por"
    foes.each_with_index do |foe, i|
      memb.push(wildBattle? ? foe.name : foe.full_name)
      text += ", " if i > 0 && i < foes.length - 1
      text += " y " if i == foes.length - 1 && foes.length > 1
      text += "{#{i+1}}"
    end
    text += wildBattle? ? " ¡apareció!" : "!"
    ...
    for side in [1, 0]
      ...
      trainers.each_with_index do |t, i|
        ...
        sent.each_with_index do |foe, j|
          ...
          text += " y " if j == sent.length - 1 && sent.length > 1
          text += "{#{j+2}}"
        end
        text += "!"
