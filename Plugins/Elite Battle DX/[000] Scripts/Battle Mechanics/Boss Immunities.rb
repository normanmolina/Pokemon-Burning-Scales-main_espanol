...
class PokeBattle_Battler
  ...
  def pbCanSleep?(*args)
    ...
    if self.immunity && rule
      @battle.pbDisplay(_INTL("¡La vitalidad de {1} evita que se quede dormido!", pbThis)) if args[0]
      return false
    end
  ...
  def pbCanPoison?(*args)
    ...
    if self.immunity && rule
      @battle.pbDisplay(_INTL("¡La vitalidad de {1} evita que se envenene!", pbThis)) if args[0]
      return false
    end
  ...
  def pbCanBurn?(*args)
    ...
    if self.immunity && rule
      @battle.pbDisplay(_INTL("¡La vitalidad de {1} evita que se queme!", pbThis)) if args[0]
      return false
    end
  ...
  def pbCanParalyze?(*args)
    ...
    if self.immunity && rule
      @battle.pbDisplay(_INTL("¡La vitalidad de {1} evita que se paralice!", pbThis)) if args[0]
      return false
    end
  ...
  def pbCanFreeze?(*args)
    ...
    if self.immunity && rule
      @battle.pbDisplay(_INTL("¡La vitalidad de {1} evita que se congele!", pbThis)) if args[0]
      return false
    end
  ...
  def pbCanConfuse?(*args)
    ...
    if self.immunity && rule
      @battle.pbDisplay(_INTL("¡El enfoque de {1} evita que se confunda!", pbThis)) if args[0]
      return false
    end
  ...
  def pbCanAttract?(*args)
    ...
    if self.immunity && rule
      @battle.pbDisplay(_INTL("¡La determinación de {1} evita que se sienta atraído!", pbThis)) if args[0]
      return false
    end
...
class PokeBattle_Move_0E7
  ...
  def pbFailsAgainstTarget?(*args)
    ...
    if args[1] && args[1].immunity && rule
      @battle.pbDisplay(_INTL("No afecta a {1}...", args[1].pbThis))
      return true
    end
...
class PokeBattle_Move_070
  ...
  def pbFailsAgainstTarget?(*args)
    ...
    if args[1] && args[1].immunity && rule
      @battle.pbDisplay(_INTL("¡{1} no se ve afectado!", args[1].pbThis))
      return true
    end
