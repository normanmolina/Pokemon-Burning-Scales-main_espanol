...
module EliteBattle
  ...
  def self.generateWild(data)
    ...
    EliteBattle.log.error("¡No hay especies definidas para Pokémon!") if !species
    EliteBattle.log.error("¡No hay nivel definido para Pokémon!") if !level
    # converts species to proper numeric value
    EliteBattle.log.error("¡Especies constantes no válidas!") if species.nil?
  ...
  def self.wildBattle(data, partysize = 1, canEscape = true, canLose = false, playersize = 1)
    ...
    if $Trainer.able_pokemon_count == 0 || ($DEBUG && Input.press?(Input::CTRL))
      pbMessage(_INTL("SALTANDO BATALLA...")) if $Trainer.pokemonCount>0
