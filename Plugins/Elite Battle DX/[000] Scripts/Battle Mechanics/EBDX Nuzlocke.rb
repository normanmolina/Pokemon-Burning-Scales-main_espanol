...
module EliteBattle
  ...
  def self.nuzlockeSelection
    ...
    desc = [
      _INTL("No poder revivir combatientes derrotados"),
      _INTL("Auto-borrar combatientes derrotados"),
      _INTL("Un encuentro por mapa"),
      _INTL("Ignorar especies duplicadas (línea)"),
      _INTL("Excluir estática del límite de encuentro"),
      _INTL("Excluir shiny del límite de encuentro")
    ]
    # default
    ...
    msgwindow.text = _INTL("Seleccione las Reglas de Nuzlocke que desea aplicar.")
    # main loop
    loop do
      ...
      # processes return
      if cmd < 0
        clear = pbConfirmMessage("¿Desea cancelar la selección de Nuzlocke?")
    ...
    # shows message
    msg = _INTL("Se han aplicado las reglas de Nuzlocke seleccionadas.")
    msg = _INTL("No se han aplicado reglas de Nuzlocke.") if added.length < 1
    msg = _INTL("Su selección ha sido cancelada.") if cmd < 0
    pbMessage(msg)
    Input.update
...
class PokeBattle_Battle
  ...
  def pbThrowPokeBall(*args)
    ...
    if EliteBattle.get(:nuzlocke) && data.include?(:ONEROUTE)
      ...
      map = $PokemonGlobal.nuzlockeData[$game_map.map_id]
      return pbDisplay(_INTL("¡Las reglas de Nuzlocke te impiden atrapar un Pokémon salvaje en un mapa en el que ya tuviste un encuentro!")) if !map.nil? && !static && !shiny
    end
...
def pbStartOver(*args)
  if EliteBattle.get(:nuzlocke)
    pbMessage(_INTL("\\w[]\\wm\\c[8]\\l[3]Todos tus Pokémon se han desmayado. ¡Has perdido el desafío de Nuzlocke! El desafío ahora se desactivará."))
