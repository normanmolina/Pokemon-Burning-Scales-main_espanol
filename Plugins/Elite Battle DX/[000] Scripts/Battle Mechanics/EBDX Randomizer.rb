...
module EliteBattle
  ...
  def self.startRandomizer(skip = false)
    ...
    msg = _INTL("Se han aplicado las reglas de Randomizer seleccionadas.")
    msg = _INTL("No se han aplicado reglas de Randomizer.") if added.length < 1
  ...
  def self.randomizerSelection
    ...
    desc = [
      _INTL("Randomizar equipos de entrenadores"),
      _INTL("Randomizar encuentros salvajes"),
      _INTL("Randomizar encuentros estaticos"),
      _INTL("Randomizar Pokémon regalado"),
      _INTL("Randomizar objetos")
    ]
    # default
    added = []; cmd = 0
    # creates help text message window
    msgwindow = pbCreateMessageWindow(nil, "choice 1")
    msgwindow.text = _INTL("Seleccione los modos de aleatorización que desee aplicar.")
    # main loop
    loop do
      ...
      # processes return
      if cmd < 0
        clear = pbConfirmMessage("¿Desea cancelar la selección a Randomizar?")
