...
class PokemonEvolutionScene
  ...
  def pbEvolution(cancancel = true)
    ...
    pbMessageDisplay(@sprites["msgwindow"],_INTL("\\se[]¿Qué?\r\n{1} está evolucionando.\\^",@pokemon.name)) { self.update }
    ...
    if canceled
      ...
      @sprites["msgwindow"].visible = true
      pbMessageDisplay(@sprites["msgwindow"],_INTL("Huh?\r\n¡{1} dejó de evolucionar!", @pokemon.name)) { self.update }
    else
  ...
  def createEvolved
    ...
    @sprites["msgwindow"].visible = true
    pbMessageDisplay(@sprites["msgwindow"], _INTL("\\se[]¡Felicidades! ¡Tu {1} evolucionó a {2}!\\wt[80]", @pokemon.name, newspeciesname)) { self.update }
