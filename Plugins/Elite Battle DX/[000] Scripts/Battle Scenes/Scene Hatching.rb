...
class PokemonEggHatch_Scene
  ...
  def pbMain
    ...
    pbMessageDisplay(@sprites["msgwindow"],_INTL("\\se[]¡{1} salió del huevo!\\wt[80]",@pokemon.name)) { self.update }
    pbMessageDisplay(@sprites["msgwindow"],_INTL("¿Te gustaría ponerle un apodo al recién nacido {1}?",@pokemon.name)) { self.update }
    if pbShowCommands(@sprites["msgwindow"],cmd,1,0) { self.update } == 0
      nickname = pbEnterPokemonName(_INTL("¿El apodo de {1}?",@pokemon.name),0,10,"",@pokemon,true)
