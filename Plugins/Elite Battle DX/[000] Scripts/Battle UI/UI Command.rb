...
class CommandWindowEBDX
  ...
  def compileCommands(index)
    ...
    if @safaribattle
      @indexes = [0,1,2,3]
      return [_INTL("BOLA"), _INTL("CARNADA"), _INTL("ROCA"), _INTL("HUIR")]
    end
    ...
    default = [_INTL("PELEA"), _INTL("MOCHILA"), _INTL("EQUIPO"), _INTL("HUIR")]
    default.push(_INTL("DEBUG")) if $DEBUG && default.length == 4 && EliteBattle::SHOW_DEBUG_FEATURES
    for i in 0...default.length
      val = default[i]; val = _INTL("LLAMAR") if default[i] == _INTL("HUIR") && (poke.shadowPokemon? && poke.inHyperMode?)
      if !d1.nil?
