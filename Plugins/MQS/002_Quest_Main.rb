...
class Player_Quests
  ...
  def activateQuest(quest,color,story)
    if !quest.is_a?(Symbol)
      raise _INTL("El argumento 'Misión' debe ser un símbolo, p. ':Misión1'.")
    end
  ...
  def failQuest(quest,color,story)
    ...
    for i in 0...@active_quests.length
      if @active_quests[i].id == quest
        ...
        found = true
        pbMessage(_INTL("\\se[{1}]<ac><c2=#{colorQuest("red")}>¡Misión fallida!</c2>\n¡Tu registro de misiones se ha actualizado!</ac>",QUEST_FAIL))
        break
      end
    end
    if !found
  ...
  def completeQuest(quest,color,story)
    ...
    for i in 0...@active_quests.length
      if @active_quests[i].id == quest
        ...
        @active_quests.delete_at(i)
        found = true
        pbMessage(_INTL("\\se[{1}]<ac><c2=#{colorQuest("red")}>¡Misión completada!</c2>\n¡Tu registro de misiones se ha actualizado!</ac>",QUEST_JINGLE))
        break
      end
    end
    if !found
  ...
  def advanceQuestToStage(quest,stageNum,color,story)
    ...
    for i in 0...@active_quests.length
      if @active_quests[i].id == quest
        ...
        found = true
        pbMessage(_INTL("\\se[{1}]<ac><c2=#{colorQuest("red")}>¡Nueva tarea añadida!</c2>\n¡Tu registro de misiones se ha actualizado!</ac>",QUEST_JINGLE))
      end
      return if found
    end
    if !found
