...
class PokemonStorageScene
  ...
  def pbUpdateOverlay(selection,party=nil)
    ...
    pbDrawTextPositions(overlay,[
       [_INTL("EQUIPO: {1}",(@storage.party.length rescue 0)),274,338,2,buttonbase,buttonshadow],
       [_INTL("Salir"),450,338,2,buttonbase,buttonshadow],
    ])
    pokemon = nil
    if @screen.pbHeldPokemon
...
class PokemonStorageScreen
  ...
  def pbStartScreen(command)
    ...
    if command==0   # Organise
      ...
      loop do
        ...
        if selected==nil
          if pbHeldPokemon
            pbDisplay(_INTL("¡Estas sosteniendo a un Pokémon!"))
            next
          end
          next if pbConfirm(_INTL("¿Continuar con las operaciones de la Caja?"))
          break
  ...
  def pbWithdraw(selected,heldpoke)
    ...
    index = selected[1]
    if box==-1
      raise _INTL("No puede retirar del equipo...");
    end
  ...
  def pbStore(selected,heldpoke)
    ...
    index = selected[1]
    if box!=-1
      raise _INTL("No se puede depositar desde Caja...")
    end
    if pbAbleCount<=1 && pbAble?(@storage[box,index]) && !heldpoke
      pbPlayBuzzerSE
      pbDisplay(_INTL("¡Ese es tu último Pokémon!"))
    elsif heldpoke && heldpoke.mail
      pbDisplay(_INTL("Por favor, elimine el correo."))
    elsif !heldpoke && @storage[box,index].mail
      pbDisplay(_INTL("Por favor, elimine el correo."))
    else
      loop do
        destbox = @scene.pbChooseBox(_INTL("¿En qué caja deposito?"))
        if destbox>=0
  ...
  def pbPlace(selected)
    ...
    if box!=-1 && index>=@storage.maxPokemon(box)
      pbDisplay("No puedo colocar eso allí.")
      return
    end
    if box!=-1 && @heldpkmn.mail
      pbDisplay("Por favor, elimine el correo.")
      return
    end
    if box>=0
  ...
  def pbRelease(selected,heldpoke)
    ...
    command = pbShowCommands(_INTL("¿Liberar a este Pokémon?"),[_INTL("No"),_INTL("Si")])
    if command==1
      ...
      @scene.pbRefresh
      pbDisplay(_INTL("{1} fue liberado.",pkmnname))
  ...
  def pbItem(selected,heldpoke)
    ...
    pokemon = (heldpoke) ? heldpoke : @storage[box,index]
    if pokemon.egg?
      pbDisplay(_INTL("Los huevos no pueden contener objetos."))
      return
    elsif pokemon.mail
      pbDisplay(_INTL("Por favor, elimine el correo."))
      return
    ...
    if pokemon.item
      ...
      if pbConfirm(_INTL("¿Tomar este {1}?",itemname))
        if !$PokemonBag.pbStoreItem(pokemon.item)
          pbDisplay(_INTL("No se puede almacenar el {1}.",itemname))
        else
          pbDisplay(_INTL("Tomó el {1}.",itemname))
          pokemon.item = nil
          @scene.pbHardRefresh
        end
      end
      ...
      if item
        ...
        pbDisplay(_INTL("{1} ahora está retenido.",itemname))
        @scene.pbHardRefresh
      end
    end
  end
  ...
  def pbBoxCommands
    commands = [
       _INTL("Salta"),
       _INTL("Fondo de pantalla"),
       _INTL("Nombre"),
       _INTL("Cancelar"),
    ]
    command = pbShowCommands(
       _INTL("¿Qué es lo que quieres hacer?"),commands)
    case command
    when 0
    ...
    when 1
      ...
      wpaper = pbShowCommands(_INTL("Elige el fondo de pantalla."),papers[0],index)
      if wpaper>=0
  ...
  def pbChoosePokemon(_party=nil)
    ...
    loop do
      ...
      if selected==nil
        next if pbConfirm(_INTL("¿Continuar con las operaciones de Box?"))
        break
      elsif selected[0]==-4   # Box name
        pbBoxCommands
      else
