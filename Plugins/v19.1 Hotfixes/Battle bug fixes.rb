  ...
  proc { |ability,user,target,move,battle|
    ...
    if user.affectedByContactEffect?(PokeBattle_SceneConstants::USE_ABILITY_SPLASH)
      ...
      if PokeBattle_SceneConstants::USE_ABILITY_SPLASH
        battle.pbDisplay(_INTL("¡La habilidad de {1} se convirtió en {2}!",user.pbThis,user.abilityName))
      else
        battle.pbDisplay(_INTL("¡La habilidad de {1} se convirtió en {2} debido a {3}!",
  ...
  def pbReturnUnusedItemToBag(item,idxBattler)
    ...
    if pbOwnedByPlayer?(idxBattler)
      if $PokemonBag && $PokemonBag.pbCanStore?(item)
        ...
        raise _INTL("No se pudo devolver el objeto no utilizado a la mochila de ninguna manera.")
      end
    else
...
class PokeBattle_Move_003 < PokeBattle_SleepMove
  def pbEndOfMoveUsageEffect(user,targets,numHits,switchedBattlers)
    ...
    user.pbChangeForm(newForm,_INTL("¡{1} transformado!",user.pbThis))
  end
end
...
class PokeBattle_Move_05E < PokeBattle_Move
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("¡{1} transformado en el tipo {2}!",user.pbThis,typeName))
  end
end
...
class PokeBattle_Move_0F1 < PokeBattle_Move
  def pbEffectAfterAllHits(user,target)
    ...
    @battle.pbDisplay(_INTL("¡{1} robó {3} de {2}!",user.pbThis,target.pbThis(true),itemName))
...
class PokeBattle_Move_0F2 < PokeBattle_Move
  def pbEffectAgainstTarget(user,target)
    ...
    @battle.pbDisplay(_INTL("¡{1} intercambió elementos con su oponente!",user.pbThis))
...
class PokeBattle_Move_0F3 < PokeBattle_Move
  def pbEffectAgainstTarget(user,target)
    ...
    @battle.pbDisplay(_INTL("¡{1} recibió {2} de {3}!",target.pbThis,itemName,user.pbThis(true)))
    target.pbHeldItemTriggerCheck
  end
end
  ...
  proc { |ability,user,targets,move,battle|
    ...
    targets.each do |b|
      ...
      if b.hasActiveAbility?(:STICKYHOLD)
        ...
        if PokeBattle_SceneConstants::USE_ABILITY_SPLASH
          battle.pbDisplay(_INTL("¡El artículo de {1} no puede ser robado!",b.pbThis))
        end
      ...
      if PokeBattle_SceneConstants::USE_ABILITY_SPLASH
        battle.pbDisplay(_INTL("¡{1} robó {3} de {2}!",user.pbThis,
           b.pbThis(true),user.itemName))
      else
        battle.pbDisplay(_INTL("¡{1} robó {3} de {2} con {4}!",user.pbThis,
...
BattleHandlers::TargetAbilityAfterMoveUse.add(:PICKPOCKET,
  proc { |ability,target,user,move,switched,battle|
    ...
    if user.hasActiveAbility?(:STICKYHOLD)
      ...
      if PokeBattle_SceneConstants::USE_ABILITY_SPLASH
        battle.pbDisplay(_INTL("¡El artículo de {1} no puede ser robado!",user.pbThis))
      end
    ...
    battle.pbDisplay(_INTL("¡{1} ha robado los {3} de {2}!",target.pbThis,
...
BattleHandlers::TargetItemOnHit.add(:STICKYBARB,
  proc { |item,user,target,move,battle|
    ...
    battle.pbDisplay(_INTL("¡El {2} de {1} fue transferido a {3}!",
...
class PokeBattle_Battler
  def pbSymbiosis
    ...
    @battle.pbPriority(true).each do |b|
      ...
      if PokeBattle_SceneConstants::USE_ABILITY_SPLASH
        @battle.pbDisplay(_INTL("¡{1} compartió su {2} con {3}!",
           b.pbThis,b.itemName,pbThis(true)))
      else
        @battle.pbDisplay(_INTL("¡El {2} de {1} permite que comparta su {3} con {4}!",
