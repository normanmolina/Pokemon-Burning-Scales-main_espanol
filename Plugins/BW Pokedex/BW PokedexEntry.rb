    ...
    ret.each do |entry|
      if !entry[0] || entry[0].empty?   # Necessarily applies only to form 0
        ...
        when 1 then entry[0] = _INTL("Hembra")
          ...
          entry[0] = (multiple_forms) ? _INTL("Una forma") : _INTL("Sin genero")
        end
      end
...
# or if the player is seeing the "normal" Dex Entry page on the Pokédex.
  if @brief
    textpos = [
       [_INTL("Registro de Pokémon completo"), 82, -2, 0, Color.new(255, 255, 255), Color.new(165, 165, 173)],
       [_INTL("{1}{2} {3}", indexText, " ", species_data.name),
          272, 54, 0, Color.new(82, 82, 90), Color.new(165, 165, 173)],
       [_INTL("Altura"), 288, 170, 0, base, shadow],
       [_INTL("Peso"), 288, 200, 0, base, shadow]
    ]
  else
    textpos = [
       [_INTL("{1}{2} {3}", indexText, " ", species_data.name),
          272, 16, 0, Color.new(82, 82, 90), Color.new(165, 165, 173)],
       [_INTL("Altura"), 288, 132, 0, base, shadow],
       [_INTL("Peso"), 288, 162, 0, base, shadow]
    ]
  end
...
# Pokémon, but did'nt capture it yet.    
  ...
  @sprites["infoverlay"].setBitmap(_INTL("Graphics/Pictures/Pokedex/map_overlay"))
    ...
    if points.length==0
      ...
      textpos.push([_INTL("Area desconocida"),Graphics.width/2,146,2,base,shadow])
    end
