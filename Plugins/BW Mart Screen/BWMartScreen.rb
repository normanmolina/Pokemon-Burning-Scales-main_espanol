...
class Window_PokemonMart < Window_DrawableCommand
  ...
  def drawItem(index, count, rect)
    ...
    ypos = rect.y
    if index == count-1
      textpos.push([_INTL("CANCELAR"), rect.x, ypos - 4, false, self.baseColor, self.shadowColor])
    else
      item = @stock[index]
  ...
  def pbRefresh
    if @subscene
      ...
      @sprites["itemtextwindow"].text =
         (itemwindow.item) ? @adapter.getDescription(itemwindow.item) : _INTL("Deja de comprar.")
      itemwindow.refresh
    end
  ...
  def pbChooseNumber(helptext,item,maximum)
    ...
    using(numwindow = Window_AdvancedTextPokemon.new("")) {   # Showing number of items
      ...
      using(inbagwindow = Window_AdvancedTextPokemon.new("")) {   # Showing quantity in bag
        ...
        inbagwindow.text = _INTL("En mochila:<r>{1}  ", qty)
  ...
  def pbBuyScreen
    ...
    loop do
      ...
      if GameData::Item.get(item).is_important?
        if !pbConfirm(_INTL("Seguramente. Quieres {1}. Eso será ${2}. ¿OK?",
        ...
        quantity=@scene.pbChooseNumber(
           _INTL("{1}? Seguramente. ¿Cuantos te gustaria?",itemname),item,maxafford)
        next if quantity==0
        price*=quantity
        if !pbConfirm(_INTL("{1}, y quieres {2}. Eso será ${3}. ¿OK?",
      ...
      if added!=quantity
        added.times do
          if !@adapter.removeItem(item)
            raise _INTL("Error al eliminar los elementos almacenados")
          end
        end
        pbDisplayPaused(_INTL("No tienes más espacio en la mochila."))
      else
        ...
        @stock.compact!
        pbDisplayPaused(_INTL("¡Aquí tienes! ¡Gracias!")) { pbSEPlay("Mart compra artículo") }
        if $PokemonBag
          if quantity>=10 && GameData::Item.get(item).is_poke_ball? && GameData::Item.exists?(:PREMIERBALL)
            if @adapter.addItem(GameData::Item.get(:PREMIERBALL))
              pbDisplayPaused(_INTL("También lanzaré un Premier Ball."))
            end
          end
        end
      end
  ...
  def pbSellScreen
    ...
    loop do
      ...
      if !@adapter.canSell?(item)
        pbDisplayPaused(_INTL("¿{1}? Oh, no. No puedo comprar eso.",itemname))
        next
      end
      qty=@adapter.getQuantity(item)
      next if qty==0
      ...
      if qty>1
        qty=@scene.pbChooseNumber(
           _INTL("¿{1}? ¿Cuántos te gustaría vender?",itemname),item,qty)
      end
      if qty==0
        @scene.pbHideMoney
        next
      ...
      price/=2
      price*=qty
      if pbConfirm(_INTL("Puedo pagar ${1}. ¿Estaría bien?",price.to_s_formatted))
        @adapter.setMoney(@adapter.getMoney+price)
        qty.times do
          @adapter.removeItem(item)
        end
        pbDisplayPaused(_INTL("Entregó el {1} y recibió ${2}.",itemname,price.to_s_formatted)) { pbSEPlay("Mart buy item") }
        @scene.pbRefresh
      end
      @scene.pbHideMoney
    end
...
def pbPokemonMart(stock,speech=nil,cantsell=false)
  ...
  cmd = pbMessage(
     speech ? speech : _INTL("¡Bienvenidos! ¿Cómo puedo servirte?"),
     commands,cmdQuit+1)
  loop do
    if cmdBuy>=0 && cmd==cmdBuy
      ...
      pbMessage(_INTL("¡Por favor ven de nuevo!"))
      break
    end
    cmd = pbMessage(_INTL("¿Hay algo más en lo que pueda ayudarte?"),
       commands,cmdQuit+1)
  end
